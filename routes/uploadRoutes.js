...
router.all('/upload', (req, resp) => {
  ...
  form.parse(req, function(err, fields, files) {
    ...
    // fileData see the formidable File interface
    // https://github.com/node-formidable/formidable?tab=readme-ov-file#file
    const { data: [fileData] } = files

    const s = '\\'
    const fileName = fileData.filepath.split(s) ? fileData.filepath.split(s)[fileData.filepath.split(s).length - 1] : ""

    folderIsExit(folder).then(val => {
      console.log('found upload file in: ', folder)

      let destFile = path.resolve(folder, index)
      console.log(`receiving the ${index} part of ${fileName}, pieces: ${fileData.newFilename}`)
      copyFile(fileData.filepath, destFile).then(
        successLog => {
          console.log(`remove the ${index} pieces to ${destFile}`)
          console.log(`===============================================================`)
          resp.send({
            stat: 1,
            desc: index
        ...
        errorLog => {
          console.log('fail to copy splice file')
          resp.send({
            stat: 0,
            desc: 'Error'
          })
...
async function getChunkList(filePath, folderPath, callback) {
  const s = '\\'
  const originFileName = filePath.split(s) ? filePath.split(s)[filePath.split(s).length - 1] : ""
  let isFileExit = await isExist(filePath)
  let result = {}
  // 如果文件(文件名, 如:node-v7.7.4.pkg)已在存在, 不用再继续上传, 真接秒传
  if (isFileExit) {
    console.log(`found ${originFileName} file in ${folderPath}` )
    console.log(`finish, return file path ${filePath}`)
    result = {
      stat: 1,
      file: {
        isExist: true,
    ...
    let fileList = []
    if (isFolderExist) {
      console.log(`found incomplete ${originFileName} file in ${folderPath}` )
      fileList = await listDir(folderPath)
      console.log(`has ${fileList.length} pieces in local, there are -> ${fileList}` );
    }
    result = {
      stat: 1,
      chunkList: fileList,
